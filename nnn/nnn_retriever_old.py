...
class NNNRetriever:
    def __init__(
        ...
        if distribution_normalization:
            ...
            retrieval_embeds = (
                retrieval_embeds - dn_lambda * self.retrieval_dev_embeds_avg
            )
        ...
        if use_faiss_reference:
            self.setup_reference_faiss_index(
                reference_embeds, faiss_reference_index, use_gpu, gpu_id
            )
        ...
        if use_faiss_reference and not use_faiss_retrieval:
            self.reference_mean_scores = torch.tensor(
                reference_mean_scores, device=self.device
            )
        ...
        if use_faiss_retrieval:
            self.setup_retrieval_faiss_index(
                retrieval_embeds, faiss_retrieval_index, use_gpu, gpu_id
            )

    def setup_reference_faiss_index(
        self, reference_embeds, faiss_reference_index, use_gpu, gpu_id
    ):
        if faiss_reference_index != None:
            if faiss_reference_index.metric_type != faiss.METRIC_INNER_PRODUCT:
                ...
                raise Exception(
                    f"FAISS reference index must be of size {self.embed_size}"
                )
            else:
            ...
            cpu_index = faiss.IndexIVFFlat(
                quantizer, self.embed_size, nlist, faiss.METRIC_INNER_PRODUCT
            )
            if use_gpu:
    ...
    def setup_retrieval_faiss_index(
        self, retrieval_embeds, faiss_retrieval_index, use_gpu, gpu_id
    ):
        if faiss_retrieval_index != None:
            if faiss_retrieval_index.metric_type != faiss.METRIC_INNER_PRODUCT:
                ...
                raise Exception(
                    f"FAISS retrieval index must have size {self.embed_size + 1}!"
                )
            else:
        ...
        modified_retrieval_embeds = np.concatenate(
            [retrieval_embeds, self.reference_mean_scores], axis=1
        )
    ...
    def compute_alignment_means_faiss(self, alternate_ks):
        batch_reference_scores, indices = self.faiss_reference_index.search(
            self.retrieval_embeds, alternate_ks
        )
    ...
    def compute_alignment_means_exhaustive(self, alternate_ks):
        ...
        for i in tqdm(range(0, self.torch_retrieval_embeds.shape[0], self.batch_size)):
            batch_reference_similarity_scores = torch.einsum(
                "ik,jk->ij",
                self.torch_retrieval_embeds[i : i + self.batch_size, :],
    ...
    def search(self, batch_query: np.matrix, top_k):
        ...
        if self.use_faiss_retrieval:
            ...
            return self.search_exhaustive(
                torch.tensor(batch_query, device=self.device), top_k
            )
    ...
    def search_faiss(self, batch_query: np.matrix, top_k, alt_weight=None):
        ...
        batch_query = np.concatenate(
            [batch_query, -alt_weight_to_use * np.ones((batch_query.shape[0], 1))],
            axis=1,
        )
    ...
    def search_exhaustive(self, batch_query: torch.tensor, top_k, alt_weight=None):
        ...
        print(
            "DIMENSIONS ARE",
            batch_query.shape,
            self.torch_retrieval_embeds.shape,
            self.reference_mean_scores.shape,
        )
        for i in tqdm(range(0, batch_query.shape[0], self.batch_size)):
            batch_similarity_scores = (
                torch.einsum(
                    "ik,jk->ij",
                    batch_query[i : i + self.batch_size, :],
                    self.torch_retrieval_embeds,
                )
                - alt_weight_to_use * self.reference_mean_scores.T
            )
