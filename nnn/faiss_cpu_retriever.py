...
class FaissCPURetriever(Retriever):
    ...
    def compute_alignment_means(self, 
        ...
        self.check_dimensions(retrieval_embeds)
        batch_reference_scores, indices = self.reference_index.search(retrieval_embeds.numpy(), alternate_ks)
    ...
    def check_dimensions(self, 
        ...
        if self.reference_index.d != retrieval_embeds.shape[1]:
            raise Exception("Reference index embedding size does not match with reference embeds!")
        if self.retrieval_index.d != retrieval_embeds.shape[1] + 1:
            raise Exception("Retrieval index embedding size does not match with retrieval embeds!")